...
def main():
    ...
    # 都市部か田舎部の判定
    text = "Answer 1 if your car is at the intersection and 0 if not."
    
    # file_paths, textをもとにQwen2に入力するためのメッセージの作成
    messages = make_messages(file_paths_mini, text)
    
    # 作成したメッセージをバッチサイズに変更
    batch_messages, batch_file_paths = change_messages_to_batch(messages, file_paths_mini, 3)
    ...
    new_intersection = args.output  / "intersection"
    new_not_intersection = args.output / "not_intersection"
    # サブフォルダを作成（os.makedirs を使う場合）
    new_intersection.mkdir(exist_ok=True, parents=True)
    new_not_intersection.mkdir(exist_ok=True, parents=True)
    ...
    for i, output in enumerate(output_list):
        ...
        # 保存先フォルダを決定
        if "1" in label:
            save_path = new_intersection / img_filename
        else:
            save_path = new_not_intersection / img_filename
